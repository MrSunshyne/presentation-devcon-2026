...
Both boxes are editable. The default schema already shows three types: string, integer, and an enum - the model can only pick a sentiment from the list. Live edits: add "artist": { "type": "string" } plus required and rerun; or change "in English" to "in French" in the system prompt - persona vs contract in one demo. Works in Kreol too (it lives next door to French) - invite the room to rewrite the message.
-->
